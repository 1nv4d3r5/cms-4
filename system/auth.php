...
class Auth
{
    ...
    public function user($user = null)
    {
        if ($user === null)
        {
            ...
            if ($this->user === null)
            {
                $current_cookie = Request::Initial()->cookie($this->cookie_name);
                // Check auth cookie
                if (strlen($current_cookie) == 80)
                {
                    $this->user = Database::current()
                                        ->Query("SELECT * FROM `cms_auth` "
                                            ...
                                            . Database::current()->Escape($current_cookie)
                                            . "' LIMIT 1")
                                        ->Fetch();

                    if ($this->user)
                        // Update cookie to expire 24 hours from now
                        setcookie($this->cookie_name, $current_cookie, time() + 36400, CMS::base_url());
                    else
                        setcookie($this->cookie_name, '', time() - 86400, CMS::base_url());
                }
                else
